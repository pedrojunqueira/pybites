...
def _obj_to_int(obj):
    if isinstance(obj, str):
        if obj not in blood_type_text.keys():
            raise ValueError
        return blood_type_text.get(obj).value
    if isinstance(obj, Bloodtype):
        return obj.value
...
def check_bt(donor, recipient):
    ...
    _check_value(donor)
    _check_value(recipient)
    compability = _particular_antigen_comp(donor, recipient)
    return -1 not in compability
